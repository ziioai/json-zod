/**
 * 将 json-zod 对象转换为 Zod Schema 的核心功能
 */

import { z } from 'zod/v4';
import type { ZodJsonRoot, ZodJsonParam, ZodJson, ZodJsonRef, ZodJsonFunctionSpec } from './types.js';
  ...
  const parseNode = (node: ZodJsonParam): any => {
    ...
    if ('_is' in node && node._is === 'ref') {
      ...
      const lazySchema = z.lazy(() => {
        ...
        if (!definition) {
          throw new Error(`[json-zod] Definition not found for ref: "${refId}" during lazy evaluation`);
        }
